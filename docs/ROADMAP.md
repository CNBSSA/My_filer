...
- [x] **P9.7** — Corporate (CIT) filing flow: `filing/corporate_{schemas,serialize,audit,service}.py` mirror the NGO scaffold — CAC Part-A-keyed taxpayer, CIT computation via placeholder bands, 10-check Audit Shield (RC validity, name, future year, revenue/turnover consistency, heavy-loss warn, declaration, doc-attach, placeholder banner), PDF via the Phase 4 renderer shim. `POST /v1/corporate-filings` CRUD + `/audit` + `/pack` + download endpoints. Mai tools `audit_corporate_filing` + `audit_corporate_return` (registry is now 20 tools). 21 new tests; full suite 351 → 372 green.
- [x] **P9.8** — Mai Filer tools: `calc_cit`, `calc_wht`, `list_wht_classes`, `validate_ubl_envelope`, `verify_cac`, `audit_corporate_filing`, `audit_corporate_return`. Registry is now 20 tools. `compose_einvoice` and `submit_mbs` land with P9.5 / P9.4.
- [x] **P9.9** — Web: `/sme` is now a full corporate intake flow (CAC-keyed taxpayer, revenue + expense line editors, WHT-already-suffered, declaration) that creates a `tax_kind='cit'` filing and redirects to `/corporate-filings/[id]`, a review page mirroring `/ngo-filings/[id]` with audit-shield loop, summary, and PDF + JSON download links. The preview calculators (`/v1/sme/calc-cit`, `/v1/sme/calc-wht`, `/v1/sme/wht-classes`, `/v1/sme/validate-ubl`) remain in place as drafting aids. Placeholder banner still prominent until 2026 statutory data lands. a11y harness (6 routes) clean; Next build green.
